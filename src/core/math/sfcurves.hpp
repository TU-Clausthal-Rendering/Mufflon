...
inline CUDA_FUNCTION constexpr u64 part_by_two16(const u16 x) {
	u64 r = x;
...
inline CUDA_FUNCTION u64 part_by_two21(const u32 x) {
...
inline CUDA_FUNCTION u32 part_by_two10(const u32 x) {
...
inline CUDA_FUNCTION constexpr u64 gray_to_binary(u64 num) {
	num = num ^ (num >> 32);
	num = num ^ (num >> 16);
	num = num ^ (num >> 8);
	num = num ^ (num >> 4);
...
inline CUDA_FUNCTION constexpr u64 binary_to_gray(u64 num) {
	return num ^ (num >> 1);
}

// Encodes 3 16-bit values into a single 64 bit value on a Z-curve.
inline CUDA_FUNCTION constexpr u64 get_morton_code(const u16 a, const u16 b, const u16 c) {
...
inline CUDA_FUNCTION constexpr u64 get_hilbert_code(const u16 a, const u16 b, const u16 c) {
