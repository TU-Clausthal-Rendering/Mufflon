...
std::pair<Polygons::FaceHandle, Polygons::FaceHandle> Polygons::vertex_split(const VertexHandle v0, const VertexHandle v1,
	...
	OpenMesh::HalfedgeHandle v1vl, vlv1, vrv1, v0v1;
	std::pair<FaceHandle, FaceHandle> faces;

	// build loop from halfedge v1->vl
	if(vl.is_valid()) {
		v1vl = m_meshData->find_halfedge(v1, vl);
	...
	if(vr.is_valid()) {
		vrv1 = m_meshData->find_halfedge(vr, v1);
	...
	if(!vl.is_valid())
		vlv1 = m_meshData->prev_halfedge_handle(m_meshData->halfedge_handle(v1));
	if(!vr.is_valid())
		vrv1 = m_meshData->prev_halfedge_handle(m_meshData->halfedge_handle(v1));

	// split vertex v1 into edge v0v1
	insert_edge(v0, vlv1, vrv1);

	// Expand the index buffer
	const std::size_t newTris = (vl.is_valid() ? 1u : 0u) + (vr.is_valid() ? 1u : 0u);
	this->reserve_index_buffer<Device::CPU>(3u * (m_triangles + newTris) + 4u * m_quads);
	auto indexBuffer = m_indexBuffer.get<IndexBuffer<Device::CPU>>().indices;
	std::size_t currIndexCount = 3u * m_triangles;
	// Shift back any quads there might be
	if(m_quads > 0u && newTris > 0u) {
		std::memmove(&indexBuffer[currIndexCount + 3u * newTris], &indexBuffer[currIndexCount], 4u * sizeof(u32) * m_quads);
		m_indexFlags.mark_changed(Device::CPU);
	}
	if(vl.is_valid()) {
		faces.first = m_meshData->face_handle(v1vl);
		// Imitate the vertex order used by OpenMesh (vl -> v0 -> v1)
		indexBuffer[currIndexCount + 0u] = static_cast<u32>(vl.idx());
		indexBuffer[currIndexCount + 1u] = static_cast<u32>(v0.idx());
		indexBuffer[currIndexCount + 2u] = static_cast<u32>(v1.idx());
		currIndexCount += 3u;
		m_indexFlags.mark_changed(Device::CPU);
	}
	if(vr.is_valid()) {
		faces.second = m_meshData->face_handle(vrv1);
		// Imitate the vertex order used by OpenMesh (v1 -> v0 -> vr)
		indexBuffer[currIndexCount + 0u] = static_cast<u32>(vl.idx());
		indexBuffer[currIndexCount + 1u] = static_cast<u32>(v0.idx());
		indexBuffer[currIndexCount + 2u] = static_cast<u32>(v1.idx());
		m_indexFlags.mark_changed(Device::CPU);
	...
	// TODO: slow, hence replace with reserve
	m_faceAttributes.resize(m_faceAttributes.get_attribute_elem_count() + newTris);
	m_triangles += newTris;

	return faces;
}

OpenMesh::HalfedgeHandle Polygons::insert_loop(const OpenMesh::HalfedgeHandle hh) {
	OpenMesh::HalfedgeHandle  h0(hh);
	OpenMesh::HalfedgeHandle  o0(m_meshData->opposite_halfedge_handle(h0));

	OpenMesh::VertexHandle    v0(m_meshData->to_vertex_handle(o0));
	OpenMesh::VertexHandle    v1(m_meshData->to_vertex_handle(h0));

	OpenMesh::HalfedgeHandle  h1 = m_meshData->new_edge(v1, v0);
	OpenMesh::HalfedgeHandle  o1 = m_meshData->opposite_halfedge_handle(h1);

	OpenMesh::FaceHandle      f0 = m_meshData->face_handle(h0);
	OpenMesh::FaceHandle      f1 = m_meshData->new_face();

	// halfedge -> halfedge
	m_meshData->set_next_halfedge_handle(m_meshData->prev_halfedge_handle(h0), o1);
	m_meshData->set_next_halfedge_handle(o1, m_meshData->next_halfedge_handle(h0));
	m_meshData->set_next_halfedge_handle(h1, h0);
	m_meshData->set_next_halfedge_handle(h0, h1);

	// halfedge -> face
	m_meshData->set_face_handle(o1, f0);
	m_meshData->set_face_handle(h0, f1);
	m_meshData->set_face_handle(h1, f1);

	// face -> halfedge
	m_meshData->set_halfedge_handle(f1, h0);
	if(f0.is_valid())
		m_meshData->set_halfedge_handle(f0, o1);
...
OpenMesh::HalfedgeHandle Polygons::insert_edge(const OpenMesh::VertexHandle vh, const OpenMesh::HalfedgeHandle h0,
	...
	m_meshData->set_next_halfedge_handle(h0, v0v1);
	...
	m_meshData->set_face_handle(v1v0, m_meshData->face_handle(h1));


	// face -> halfedge
	if(m_meshData->face_handle(v0v1).is_valid())
		m_meshData->set_halfedge_handle(m_meshData->face_handle(v0v1), v0v1);
