...
inline CUDA_FUNCTION MatSampleLambert fetch(const textures::ConstTextureDevHandle_t<CURRENT_DEV>* /*textures*/,
...
inline CUDA_FUNCTION math::PathSample sample(const MatSampleLambert& params,
...
inline CUDA_FUNCTION math::BidirSampleValue evaluate(const MatSampleLambert& params,
											  const Direction& incidentTS,
...
inline CUDA_FUNCTION Spectrum albedo(const MatSampleLambert& params) {
	return params.albedo;
}

inline CUDA_FUNCTION math::SampleValue emission(const MatSampleLambert& /*params*/, const scene::Direction& /*geoN*/, const scene::Direction& /*excident*/) {
	return math::SampleValue{};
}

inline CUDA_FUNCTION float pdf_max(const MatSampleLambert& /*params*/) {
	return 1.0f / ei::PI;
}
