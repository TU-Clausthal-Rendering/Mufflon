...
public:
	...
	float get_density_interpolated(const ei::Vec3& pos, const ei::Vec3& normal, ei::Vec3* gradient = nullptr) const;
...
private:
	...
	int increment_if_positive(const int idx);

	// Returns the new child pointer or 0
	int split_node_if_necessary(const int idx, const int count, const int currentDepth);

	// Non-atomic unconditional split. Returns the new address
	int split(const int idx);
};
