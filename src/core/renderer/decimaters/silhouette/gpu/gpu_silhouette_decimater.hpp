...
class GpuImportanceDecimater {
public:
	using Mesh = scene::geometry::PolygonMeshType;
	using VertexHandle = typename Mesh::VertexHandle;

	GpuImportanceDecimater(scene::Lod& original, scene::Lod& decimated,
	...
	GpuImportanceDecimater(const GpuImportanceDecimater&) = delete;
	GpuImportanceDecimater(GpuImportanceDecimater&&);
	GpuImportanceDecimater& operator=(const GpuImportanceDecimater&) = delete;
	GpuImportanceDecimater& operator=(GpuImportanceDecimater&&) = delete;
	~GpuImportanceDecimater();

	void upload_normalized_importance();

	// Resizes the buffers properly
	Importances<Device::CUDA>* start_iteration();
	// Updates the importance densities of the decimated mesh
	void udpate_importance_density(const ImportanceSums& impSums);
...
private:
	...
	// General stuff
	unique_device_ptr<Device::CUDA, Importances<Device::CUDA>[]> m_devImportances;		// Buffer on the GPU
	std::unique_ptr<Importances<Device::CUDA>[]> m_importances;							// Buffer for copying from GPU
