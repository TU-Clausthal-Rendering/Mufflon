...
void GpuShadowSilhouettes::on_scene_load() {
...
void GpuShadowSilhouettes::on_scene_unload() {
	m_decimaters.clear();
	m_currentDecimationIteration = 0u;
}

void GpuShadowSilhouettes::post_iteration(OutputHandler& outputBuffer) {
...
void GpuShadowSilhouettes::pre_descriptor_requery() {
...
void GpuShadowSilhouettes::iterate() {
	if((int)m_currentDecimationIteration < m_params.decimationIterations) {
...
void GpuShadowSilhouettes::display_importance() {
	for(int i = 0; i < m_outputBuffer.get_num_pixels(); ++i)
		m_seeds[i] = static_cast<u32>(m_rng.next());
	copy(m_seedsPtr.get(), m_seeds.get(), sizeof(u32) * m_outputBuffer.get_num_pixels());
...
void GpuShadowSilhouettes::compute_max_importance() {
	m_maxImportance = 0.f;
...
void GpuShadowSilhouettes::initialize_decimaters() {
...
void GpuShadowSilhouettes::update_reduction_factors() {
