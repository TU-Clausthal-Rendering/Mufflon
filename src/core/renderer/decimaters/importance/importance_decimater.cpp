...
#pragma PARALLEL_REDUCTION(+, importanceSum)
	...
	logPedantic("Importance sum: ", importanceSum);
	m_importanceSum = importanceSum;
	...
	for(auto iter = m_originalMesh.vertices_begin(); iter != m_originalMesh.vertices_end(); ++iter) {
		...
		m_originalMesh.property(m_importanceDensity, vertex) = m_importances[m_originalMesh.property(m_collapsedTo, v).idx()].viewImportance.load();
	}
}
